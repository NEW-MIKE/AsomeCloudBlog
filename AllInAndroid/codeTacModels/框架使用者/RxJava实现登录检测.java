/**RxJava 的用处在，将我们之前的编写的模式做了一次更改，变成了响应式的编程，在此之前，我们采取的编程的方式是数据变了，然后
 * 检测数据是否变了，在根据数据的变动采取相应的行动，第二阶段多了一个自己实现的监听，不是别人检测到我变化了而采取行动，而是
 * 我通过注册监听的方式，来通知别人采取行动，第三阶段，采用的是响应式的，我的变化，会带来哪些影响，我是变化者，同样也是管理者
 * 我管理了由于我的变化将会导致的一系列的变化，我的任何的变化都在我的链条里面进行补充和更改。
 */
